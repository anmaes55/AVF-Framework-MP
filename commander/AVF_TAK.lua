...
function AVF_TAK_INIT()
	...
	AVF_TAK_MOVE_SPRITE = LoadSprite("MOD/gfx/AVF_TAK_MOVE_ICON.png")
	AVF_TAK_MOVE_INNER_SPRITE = LoadSprite("MOD/gfx/AVF_TAK_MOVE_INNER_ICON.png")
	AVF_TAK_MOVE_OUTER_SPRITE = LoadSprite("MOD/gfx/AVF_TAK_MOVE_OUTER_ICON.png")
	AVF_TAK_ATTACK_SPRITE= LoadSprite("MOD/sprite/reticle2.png")
	AVF_TAK_COMMAND_SPRITE= LoadSprite("MOD/sprite/reticle3.png")
end
...
function AVF_TAK_select_vehicles_in_region(x1,y1,x2,y2)
	...
	for faction_name,faction in pairs(AVF_TAK_SERVER.assets) do
		if(faction_name==current_target_side) then  
...
function AVF_TAK_DRAW(dt)
	...
	if(AVF_TAK_CONTROLLER.COMMANDER_CAM_ACTIVE) then 
		...
		AVF_TAK_DRAW_COMMAND_ICONS()
		AVF_TAK_DRAW_LAST_COMMAND(dt)
		...
		UiPush()
			...
			local key = "lmb : "
			local func = "select/deselect units"
			UiFont("bold.ttf", 22)
			UiAlign("right")
			UiText(key)
			UiTranslate(10, 0)
		...
		UiPush()
			...
			local w = 250
			local h = 1*22 + 30
			UiTranslate(UiWidth()-w-20, UiHeight()-h-20 - 100) -- because I don't know how big the official vehicle UI will be
			UiColor(0,0,0,0.5)
			UiImageBox("ui/common/box-solid-6.png", 250, h, 6, 6)
			UiTranslate(125, 32)
			UiColor(1,1,1)
		...
		UiPush()
			...
			UiTranslate(-10, 22)
		UiPop()
	elseif GetString("game.player.tool") == "avf_tak" and GetBool("game.player.canusetool") then
		UiPush()
			UiAlign("top left")
			local w = 250
			local h = 1*22 + 30
...
function AVF_TAK_GIVE_COMMAND()
	...
	if(#AVF_TAK_CONTROLLER['selected_vehicles']>0) then 
		if(InputPressed('rmb')) then 
			...
			local AVF_TAK_COMMAND = 'AVF_TAK_MOVE'
			if hit then
...
function AVF_TAK_DRAW_LAST_COMMAND(dt) 
	if(AVF_TAK_CONTROLLER.LAST_COMMAND and AVF_TAK_CONTROLLER.SHOW_COMMAND_TIMER>0) then 
		 AVF_TAK_drawCommandSprite(Transform(AVF_TAK_CONTROLLER.LAST_COMMAND_POS)) 
		 AVF_TAK_CONTROLLER.SHOW_COMMAND_TIMER = AVF_TAK_CONTROLLER.SHOW_COMMAND_TIMER - dt
	end
...
function AVF_TAK_drawCommandSprite(t)
	...
	local size = 3
	size=2+1*math.sin(GetTime()*3)
	SPRITE_COLOUR= AVF_TAK_CONTROLLER.MOVE_COMMAND_COLOUR

	DrawSprite(AVF_TAK_MOVE_INNER_SPRITE, t, size, size, SPRITE_COLOUR[1], SPRITE_COLOUR[2], SPRITE_COLOUR[3], 1, false, false)
	DrawSprite(AVF_TAK_MOVE_INNER_SPRITE, t, size, size, SPRITE_COLOUR[1], SPRITE_COLOUR[2], SPRITE_COLOUR[3], 1, true, false)

	local tr = QuatEuler(0,0,GetTime()*-80)
	t.rot = QuatRotateQuat(t.rot,tr)
	
	DrawSprite(AVF_TAK_MOVE_SPRITE, t, size, size, SPRITE_COLOUR[1], SPRITE_COLOUR[2], SPRITE_COLOUR[3], 1, false, false)
	DrawSprite(AVF_TAK_MOVE_SPRITE, t, size, size, SPRITE_COLOUR[1], SPRITE_COLOUR[2], SPRITE_COLOUR[3], 1, true, false)
	
	local tr = QuatEuler(0,0,GetTime()*100)
	t.rot = QuatRotateQuat(t.rot,tr)

	DrawSprite(AVF_TAK_MOVE_OUTER_SPRITE, t, size*1.3, size*1.3, SPRITE_COLOUR[1], SPRITE_COLOUR[2], SPRITE_COLOUR[3], 1, false, false)
	DrawSprite(AVF_TAK_MOVE_OUTER_SPRITE, t, size*1.3, size*1.3, SPRITE_COLOUR[1], SPRITE_COLOUR[2], SPRITE_COLOUR[3], 1, true, false)
end

function AVF_TAK_DRAW_COMMAND_ICONS()
	if(#AVF_TAK_CONTROLLER['selected_vehicles']>0) then 
		local x1, y1 = UiGetMousePos()	
		UiTranslate(x1+10, y1+10)
		UiColor(0,1,0)
		UiImageBox("MOD/gfx/AVF_TAK_MOVE_ICON.png", 10*3, 10*3,0,1,0,1)		
	end
end
					...
					if(distToPlayer <= displayRange) then  
						...
						UiPush()
							...
							if dist > 0 then
								...
								elseif(faction_name ==current_target_side) then 
									...
									for i,selected_asset in ipairs(AVF_TAK_CONTROLLER['selected_vehicles']) do 
										if(selected_asset == asset) then 
											...
											-- UiTranslate(colourW*.1,colourH*.1)
											-- DebugWatch("setting img box",current_target_side)
