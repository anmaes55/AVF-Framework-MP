...
function mapNode:needs_update()
	self.needs_update = true
end

function mapNode:state_updated(time)
	self.needs_update = false
...
function mapNode:getPotentialCost(came_from_y)
	-- DebugWatch("last check",self.last_check)
	if(GetTime() -  self.last_check >self.live_time) then
		local gridCost,validTerrain,avgHeight,grid_avg = nil, nil, nil, nil 
		-- DebugWatch("needs update, old cost",self.baseCost)
		self.last_check = GetTime()
		local pos = self:getPos()
		if(came_from_y <-100) then 
			gridCost,validTerrain,avgHeight,grid_avg =  scanGrid(pos[1],pos[3])
		else
			gridCost,validTerrain,avgHeight,grid_avg =  scanGrid_local(pos[1],pos[3],came_from_y)
		end
		self.validTerrain = validTerrain
		self.baseCost = gridCost 
		self:setHeight(avgHeight)
		-- DebugWatch("updated, new cost",self.baseCost)
		
	end
	return self.baseCost 
end

function mapNode:get_height_cost(y)
	return math.abs(self.y -y) 
end

function mapNode:get_last_check()
