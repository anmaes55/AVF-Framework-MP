...
function handleSniperMode(dt ) 
	...
	if(min == -180 and max == 180 ) then
		SetJointMotor(focusGun.gunJoint, (focusGun.elevationRate*100)*(current-target_y),rotation_force*3)
		
	else

		SetJointMotorTarget(focusGun.gunJoint, target_y, (focusGun.elevationRate*100),rotation_force*3)	
	end
...
function set_sniper_cam(dt) 
	...
	--[[
		...
		REWORKED FOR ACCURACY

	]]
	local zero_range = 150
	if(focusGun.zeroing and focusGun.zeroing >=50) then
		zero_range = focusGun.zeroing 
	end	

	local grav_coef = focusGun.magazines[focusGun.loadedMagazine].CfgAmmo.gravityCoef or 1
	local grav_g = globalConfig.gravity

	local t = retrieve_first_barrel_coord(focusGun)
	-- local bodyPoint = Vec(0, 0, -0.5)
	-- local p = TransformToParentPoint(t, bodyPoint)
	
	local gravity = (VecLength(grav_g) * grav_coef)
	...
	commanderPos.rot  = QuatLookAt(commanderPos.pos, expected_hit_point)
...
function initiate_missile_guidance(dt,gun,firing)
	...
	databus:update_missile_guidance(gun)					    	
end
...
function gunLaying(gun,barrelCoords,targetPos)
	...
	local target_y = gun.commander_view_y * (max - min) + min
	SetJointMotorTarget(gun.gunJoint, target_y, (gun.elevationRate*10)	,rotation_force*2)
...
function getPlayerZero()
	if InputPressed(armedVehicleControls.increase_zero) or InputDown(armedVehicleControls.increase_zero) then
	 
		return true,false,InputDown(armedVehicleControls.increase_zero)
	elseif  InputPressed(armedVehicleControls.decrease_zero) or InputDown(armedVehicleControls.decrease_zero) then
	end
