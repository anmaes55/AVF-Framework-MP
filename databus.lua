...
function databus:update_missile_guidance(gun)
	...
	if(gun.missile_guidance_current_track and MISSILE_TRACK_TIME_MIN) then 

		SetTag(gun.id,"avf.databus.TARGET_LOCKED", gun.missile_guidance_current_track>MISSILE_TRACK_TIME_MIN)
	else
		SetTag(gun.id,"avf.databus.TARGET_LOCKED",false)
	end
...
function databus:update_weapon_information(gun)
	...
	SetTag(gun.id,"avf.databus.magazineCapacity",currentMagazine.magazineCapacity)
