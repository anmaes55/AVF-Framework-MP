...
function AVF_ai:init_custom_ai_behaviours(ai)
	...
	ai.heuristic_approach = AStar.APPROACHES[math.random(1,#AStar.APPROACHES)]
 ...
 function AVF_ai:aiTick(dt)
	...
	capture_points = FindTriggers("avf_conquest_point",true)
 	...
 	for key,ai in ipairs(self.vehicles) do
 		...
 		if(ai.alive)  then 
 			if(
 				HasTag(ai.id,"avf_vehicle_disabled") or 
 				GetVehicleHealth(ai.id)<0.5 or 
 				GetTagValue(ai.id, "AVF_status")=="disabled" or
				HasTag(ai.id, "vehicle_disabled") or 
				GetTagValue(ai.id, "AVF_status")=="dead") then 
 				ai.alive = false
...
function AVF_ai:control_vehicle(ai)
	...
	pathfinding_look_ahead = math.min(math.max(1,pathfinding_look_ahead * (vel/3)),3)
