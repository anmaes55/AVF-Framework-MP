...
function handle_path_plotting(startPos,goalPos,dt,ai,vehicle_body_parts)
	...
	if(mapInitialized) then 
		local pathing_state = get_pathing_state()

		if(_AI_DEBUG_PATHING) then 
			DebugWatch("pathing state",pathing_state)
		end
		if(pathing_state =="READY") then
			init_pathfinding(startPos, goalPos,ai.heuristic_approach)
			
		elseif(pathing_state == "SEARCHING") then 
...
function drawPath(lastPath)
	...
	for i=1, #lastPath-1 do
		p1=VecAdd(VecCopy(lastPath[i]),Vec(0,1,0))
		p2=VecAdd(VecCopy(lastPath[i+1]),Vec(0,1,0))
...
function scanGrid_local(x,y,height)
	-- if(height<0 ) then 
	-- 	DebugWatch("height too low",height)
	-- elseif(height<0 ) then 
	-- 	DebugWatch("height okay",height)
	-- end
  local pos = Vec(0,0,0)
  local gridScore = 1
  local spotScore = 0 
  local scan_length =  mapSize.gridHeight*4
  local hitHeight = scan_length
  local heightOrigin = 1000000
  local minHeight = heightOrigin
  local maxHeight = -heightOrigin
  local validTerrain  = true
  local xLocal = 0
  local yLocal = 0
   -- DebugPrint("hitHeight: "..hitHeight.." scan_length: "..scan_length.." start_scan:"..height+(mapSize.gridHeight*2) )
  for y1= 1, mapSize.grid, mapSize.gridResolution do
  	yLocal = (y+y1) - mapSize.grid/2
    for x1= 1, mapSize.grid, mapSize.gridResolution do
    	xLocal = (x+x1) - mapSize.grid/2
      spotScore,hitHeight,hit =  getMaterialScore_local(xLocal, yLocal,height+(mapSize.gridHeight*2),scan_length)
      hitHeight = height +(hitHeight-(mapSize.gridHeight*2))
      if(hitHeight == scan_length or IsPointInWater(Vec(xLocal,height+hitHeight,yLocal))or not hit) then
       	-- DebugWatch("hitHeight == scan_length",hitHeight == scan_length)
       	-- DebugWatch("water check height",Vec(xLocal,hitHeight,yLocal))
       	-- DebugWatch("IsPointInWater(Vec(xLocal,hitHeight,yLocal))",IsPointInWater(Vec(xLocal,hitHeight,yLocal)))
       	-- DebugWatch("or not hit",not hit)
        minHeight = -mapSize.scanLength
        maxHeight = mapSize.scanLength
        validTerrain = false
      elseif(minHeight == heightOrigin or maxHeight == heightOrigin) then
...
function getMaterialScore_local(x,y,probe_height,scan_length)
  ...
  QueryRequire("large")
  local hit, dist,normal,shape = QueryRaycast(probe, Vec(0,-1,0), scan_length)
  if hit then
  ...
  -- DebugWatch("scan length",scan_length)
  -- DebugWatch("dist",dist)
  local hitHeight = scan_length - dist
