...
function draw_weapon_true_reticle(gun)
 UiPush()
	...
	if(gun.canZoom and  vehicle.ZOOMLEVEL>vehicle.ZOOMMIN) then
		-- draw outer region
		if(gun.fireControlComputer) then
			-- DebugWatch("gun.fireControlComputer",gun.fireControlComputer)
			UiImageBox(gun.fireControlComputer,UiWidth()*1.0,UiHeight()*1.0,1,1)
		else
			UiImageBox("MOD/gfx/scopeRegion.png",UiWidth()*1,UiHeight()*1,1,1)
			UiImageBox("MOD/gfx/scopeOutside_3.png",UiWidth()*1,UiHeight()*1,1,1)
		end

	-- get zero pos

	local zero_range = 150
	if(gun.zeroing and gun.zeroing >=50) then
		zero_range = gun.zeroing 
	end	
	local 	focusGunPos = retrieve_first_barrel_coord(gun)
	local gun_vel = gun.magazines[gun.loadedMagazine].CfgAmmo.velocity
	local gravity_vector = VecLength(VecScale(globalConfig.gravity,gun.magazines[gun.loadedMagazine].CfgAmmo.gravityCoef))
	...
	local grav_coef = gun.magazines[gun.loadedMagazine].CfgAmmo.gravityCoef or 1
	local grav_g = globalConfig.gravity

	local t = retrieve_first_barrel_coord(gun)
